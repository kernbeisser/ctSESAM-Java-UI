...
public class CtSESAM {
    ...
    private byte[] sha512HMAC(byte[] key, byte[] password) {
        try {
            ...
            Logger.getLogger(CtSESAM.class.getName()).log(Level.SEVERE, "Ui...");
            return password;
        }
    }
    ...
    private byte[] PBKDF2_HMAC_sha512(byte[] hashString, byte[] salt, int iterations) {
        int dkLen = 64;
        int hLen = 64;
    ...
    public String getPassword(byte[] hashValue,
    ...
    public String genHashedPW(String mpw, String dom, boolean sc, boolean l, boolean n) {
        String genPassword = null;
        try {
            byte[] hashString = (dom + mpw).getBytes("UTF-8");
            byte[] salt = ("pepper").getBytes("UTF-8");
            byte[] digest = PBKDF2_HMAC_sha512(hashString, salt, 4096);
            genPassword = getPassword(digest, sc, l, n, 10);
